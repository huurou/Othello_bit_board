...
class Ban:
    ...
    def draw_ban(self, board):
        #bbとwbをそれぞれ石がある場所を1と2に置き換え
        #nparrayにマージ
        marge_board = np.zeros((64))
        mask = 0x8000000000000000  # 最上位ビットのみ1
        for i in range(64):
            if board.bb & mask:
                marge_board[i] = 1
            if board.wb & mask:
                marge_board[i] = 2
            mask >>= 1
        marge_board = marge_board.reshape((8,8))
        print('   0 1 2 3 4 5 6 7')
        print('  ---------------')
        for i in range(8):
            print('%d|' % i, end='')
            for j in range(8):
                print(self.stone(marge_board[i][j]), end=' ')
            print('|')
        print('  ---------------')
...
class Play:
    ...
    def legal_board(self, pb, ob):
        ...
        #左上方向
        t = ab & (pb << 9)
        for _ in range(5): t |= ab & (t << 9)
        lb |= bl & (t << 9)
        #右上方向
        t = ab & (pb << 7)
        for _ in range(5): t |= ab & (t << 7)
        lb |= bl & (t << 7)
        #左下方向
        t = ab & (pb >> 7)
        for _ in range(5): t |= ab & (t >> 7)
...
class Move:
    ...
    def play_out(self, pb, ob, mp):  # pb:黒, ob:白
    ...
    def ucb(self, pb, ob):
        ...
        for i in range(1, SEARCH_NUM + 1):
            s = u.argmax()
            mp = ll[s]
            w[s] += Move().play_out(pb, ob, mp)
            v[s] += 1
            for j in range(n):
                u[j] = w[j] / (1 + v[j]) + 2 * math.sqrt(2 * math.log(i) / (1 + v[j]))
        return ll[v.argmax()]  # mp
class Othello:
    def cvc_kifu(self, count):
        ...
        while True:
            if not Play().legal_board(pb,ob):
                mv = 64
                kifu = np.array([ban.bb, ban.wb, color, mv], dtype='uint64')
        ...
        if bs >= ws:
            ...
            r = np.ones((row_num, 1))
            kifulist = np.append(kifulist, r, axis=1)
        np.save('./kifu/{0}-{1}'.format(datetime.now().strftime('%y_%m_%d_%H_%M_%S_%f'), count), kifulist)
